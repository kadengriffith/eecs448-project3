...
let ai_speed = player_speed;
let player_mass = 2600;
let player_friction = 0.008;
let player_restitution = 1.0;
// Puck
let puck_height = 0.3;
...
let ground_yoff = -0.5;
// Walls
let playarea_height = 4;
let boundary_height = 25;
let playarea_yoff = 1.2;
let playarea_restitution = 1.0;
let playarea_localOpacity = 0.95;
// Goal
let goal_height = 2.04;
...
function loadGameObjects(scene) {
  ...
  S1.position.y = playarea_yoff;
  // Use showPlayArea to view the boundaries
  if(showPlayArea) { S1.showBoundingBox = true; } else {
    loadMaterial(S1, "sides", loadTextures, loadTextures, [1.0, 1.0, 1.0], scene);
  ...
  S5.scaling = new BABYLON.Vector3(goal_width, playarea_height - 1.7, 1);
  S5.position.z = (-ground_length / 2);
  S5.position.y = goal_height;
  S5.rotation.y = Math.PI;
  // Use showPlayArea to view the boundaries
  if(showPlayArea) { S5.showBoundingBox = true; } else {
    loadMaterial(S5, "goals", loadTextures, loadTextures, [1.0, 1.0, 1.0], scene);
  }
  // Appears as CLOSEST CENTER bounded box
  S6 = BABYLON.MeshBuilder.CreatePlane("side6", playarea_height, scene);
  S6.scaling = new BABYLON.Vector3(goal_width, playarea_height - 1.7, 1);
  S6.position.z = ground_length / 2;
  S6.position.y = goal_height;
  // Use showPlayArea to view the boundaries
  ...
  S7 = BABYLON.MeshBuilder.CreatePlane("S7", playarea_height, scene);
  ...
  S9.scaling = new BABYLON.Vector3(ground_length / 4, boundary_height / 4, 0.005);
  ...
  S10.scaling = new BABYLON.Vector3(ground_length / 4, boundary_height / 4, 0.005);
  ...
  S11.scaling = new BABYLON.Vector3(goal_width / 8,  boundary_height / 4, 0.005);
  ...
  S12.scaling = new BABYLON.Vector3(goal_width / 8,  boundary_height / 4, 0.005);
  ...
  S13.scaling = new BABYLON.Vector3(goal_width / 4, boundary_height / 4 - goal_height / 4, 0.005);
  S13.position.z = (-ground_length / 2);
  S13.position.y = boundary_height / 2 + goal_height / 2;
  ...
  S14.scaling = new BABYLON.Vector3(goal_width / 4, boundary_height / 4 - goal_height / 4, 0.005);
  S14.position.z = ground_length / 2;
  S14.position.y = boundary_height / 2 + goal_height / 2;
  S14.visibility = 0;
  // Use showPlayArea to view the boundaries
  ...
  S15.scaling = new BABYLON.Vector3(goal_width / 8,  boundary_height / 4, 0.005);
  ...
  if(showPlayArea) { S15.showBoundingBox = true; } else {
    loadMaterial(S15, "goalRight", false, false, [1.0, 1.0, 1.0], scene);
  }
  // Appears as CLOSEST RIGHT bounded box
  S16 = BABYLON.Mesh.CreateBox("side16", playarea_height, scene);
  S16.scaling = new BABYLON.Vector3(goal_width / 8,  boundary_height / 4, 0.005);
  ...
  Camera3.lockedTarget = new BABYLON.Vector3(0, 0, 0);
  // This creates and positions a camera for the menu
